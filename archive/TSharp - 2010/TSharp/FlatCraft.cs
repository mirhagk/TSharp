...
// Creates the record that holds the blocks in a chunk
        class chunk
        {
            ...
            public block[,] blocks = new block[32, 128];
        ...
        static chunk[] chunks = new chunk[11];
        //Array of light sources(Torches)
        //X positions
        static List<int> lightsourcesx = new List<int>();
        //Y positions
        static List<int> lightsourcesy = new List<int>();
        //Chunk index
        static List<int> lightsourcechunk = new List<int>();
        //The light levels of the blocks surrounding the block being updated
        static int right, left, up, down, newlight;
        // Prevents multiples of a torch
        static int flag = 0;
        
    //Array of light sources(Torches)
    //Updated x and y coords
     static int chunk1, l, m;
    //Used to calculate the light level of a certain block
    static int d, e;//Day Time
        ...
        static int Base = floor(positionx / 16 / 32) - 5;
        //Procedure to update a blocks light level
        static void updateBlock(int id, int i, int j)
        {
            var baselight = 0;
            if (daytime != true)
            {
            ...
            else
            {
                ...
                if (chunks[id - Base].blocks[i, j].lightlevel > 10)
                {
                    chunks[id - Base].blocks[i, j].lightlevel = 10;
                }
                //If the block is a torch
                if (chunks[id - Base].blocks[i, j].blockid == 15)
                {
                    //Prevents multiple torch variables
                    if (upper(lightsourcesx) != 0)
                    {
                        if (lightsourcesx[upper(lightsourcesx)] == i && lightsourcesy[upper(lightsourcesy)] == j)
                        {
                            flag = 1;
                        }
                    }
                    ...
                    if (flag != 1)
                    {
                        ...
                        lightsourcesx.Add(new int());
                        lightsourcesy.Add(new int());
                        lightsourcechunk.Add(new int());
...
static void updateLighting (int id, int i, int j){
    //Resets the light sources arrays
    lightsourcesx.Clear();
    lightsourcesy.Clear();
    lightsourcechunk.Clear();
    //Resets all potentially effected blocks light levels
    for (int a=0;a<=20;a++){
        for (int b=j+10;b>0;b--){
            if (b < 129 && b > 0){
                if (i - 10 + a < 0)
    ...
    if (upper (lightsourcesx) != 0){
        ...
        for (int a=0;a<upper(lightsourcesx);a++){
            //Loops through the surrounding blocks
            for (int b=lightsourcesx[a]-10;b<=lightsourcesx[a]+10;b++){
                for (int c=lightsourcesy[a]-10;a<=lightsourcesy[a]+10;c++){
                    ...
                    if (chunks [chunk1].blocks [l,m].lightlevel > targetlightlevel){
...
//Generates a chunk
static void createChunk (int id){
    ...
    var heights =new float[6,32];
    //Final y positions
    var finalheights =new float[32];
    //For all 6 passes
    for (int cnt =0;cnt<6;cnt++){
        //Te frequency get lower over passes and amplitude gets larger
        frequency /= 2;
        amplitude = pow(persistence, cnt);
        ...
        if (id - 1 - Base > -1){
            if (chunks [id - 1 - Base].lighted == 1){
                for (int i=127;i>=0;i--){
                    if (chunks [id - 1 - Base].blocks [31,i].blockid == 4){
                        heightvalue = i;
                        break;
                    }
                }
        ...
        if (id + 1 - Base < 11){
            if (chunks [id + 1 - Base].lighted == 1){
                direction = "left";
                for (int i=127;i>=0;i--){
                    if (chunks [id + 1 - Base].blocks [0,i].blockid == 4){
                        heightvalue = i;
                        break;
                    }
                }
        ...
        if (direction == "right"){
            ...
            for (int i=0;i<32;i+=round(frequency)){
                ...
                for (int j=0;j<round(frequency);j++){
                    if (i + j < 32){
            ...
            for (int i=0;i<32;i+=round(frequency)){
                for (int j=0; j<round(frequency)-1;j++){
                    if (i + j < 32){
                        if (i + round (frequency) < 32)
            ...
            for (int i=30;i>=0;i-=round(frequency)){
                heights [cnt,i] = heights [cnt,i + 1] + (Rand.Real() * 2 - 1) * persistence;
                for (int j=0;j<round(frequency);j++){
                    if (i - j > -1){
                        heights [cnt,i - j] = heights [cnt,i];
            ...
            for (int i=31;i>=0;i-=round(frequency)){
                for (int j=0;j<round(frequency)-1;j++){
                    if (i - j > -1){
                        if (i - round (frequency) > -1)
                            heights [cnt,i - j] = heights [cnt,i] + ((heights [cnt,i - round (frequency)] - heights [cnt,i]) / frequency) * j;
                        else{
    ...
    for (int i=0;i<32;i++){
        finalheights [i] = round ((heights [0,i] + heights [1,i] + heights [2,i] + heights [3,i] + heights [4,i] + heights [5,i]) / 6f);
    }
    ...
    for (int i=1;i<31;i++){
        if (finalheights [i - 1] == finalheights [i + 1]){
    ...
    for (int i=0;i<32;i++){
        ...
        for (int j=127;j>=0;j--){
            if (chunks [id - Base].blocks [i,j].blockid == 1){
                groundheight = j;
                break;
            }
        }
        // Assigns air to all sky blocks
        for (int j=groundHeight;j<128;j++){
            chunks [id - Base].blocks [i,j].blockid = 0;
        }
        //IF the biome is forest
        if (chunks [id - Base].biome == 1){
            ...
            for (int j=groundheight-4;j<groundheight;j++){
                chunks [id - Base].blocks [i,j].blockid = 3;
            }}
            //If the biome is desert
        else if (chunks [id - Base].biome == 2){
...
while(true){
    ...
    recipesinvent.Add(new item[2][]{new item[2]{new item(), new item()}, new item[2]{new item(), new item()}});
    outputsinvent.Add(new item());
    //Gets the first row of items
    get(stream, ref temp1);
    //Stores the first 2 numbers as the first item, second 2 as the second item, both on the first row
    recipesinvent[upper (recipesinvent)][0][0].id=strint (temp1.Substring(0,2));
    recipesinvent[upper (recipesinvent)][0][1].id=strint (temp1.Substring(2,2));
    //Gets the second row of items
    get(stream, ref temp1);
    //Stores the first 2 numbers as the first item, second 2 as the second item, both on the first row
    recipesinvent [upper (recipesinvent)] [1] [0].id = strint (temp1.Substring(0,2));
    recipesinvent [upper (recipesinvent)] [1] [1].id = strint (temp1.Substring(2,2));
    //Gets the id and amount of the output and inserts directly into the outputs array
    get(stream, ref outputsinvent [upper (outputsinvent)].id);
    get(stream, ref outputsinvent [upper (outputsinvent)].amount);
}
...
open(ref stream, "datafiles/Recipes.txt", "r");
//Loops through the files and stores all recipes in the arrays.
while(true){
    //Stop checking for recipes once the end of the file is reached
    if ( eof (stream)) break;
    ...
    var temp = "";
    //Expands the array to fit the new recipe
    recipesinvent.Add(new item[3][]{new item[3]{new item(), new item(), new item()}, new item[3]{new item(), new item(), new item()}, new item[3]{new item(), new item(), new item()}});
    outputsinvent.Add(new item());
    //Gets the First row of items
    get(stream, ref temp);
    //Stores the first 2 numbers as the first item, second 2 as the second item, third 2 as the third item, all on the first row
    recipes [upper (recipes)] [0] [0].id = strint (temp.Substring(0,2));
    recipes [upper (recipes)] [0] [1].id = strint (temp.Substring(2,2));
    recipes [upper (recipes)] [0] [2].id = strint (temp.Substring(4,2));
    //Gets the Second row of items
    get(stream, ref temp);
    //Stores the first 2 numbers as the first item, second 2 as the second item, third 2 as the third item, all on the second row
    recipes [upper (recipes)] [1] [0].id = strint (temp.Substring(0,2));
    recipes [upper (recipes)] [1] [1].id = strint (temp.Substring(2,2));
    recipes [upper (recipes)] [1] [2].id = strint (temp.Substring(4,2));
    ...
    recipes [upper (recipes)] [2] [0].id = strint (temp.Substring(0,2));
    recipes [upper (recipes)] [2] [1].id = strint (temp.Substring(2,2));
    recipes [upper (recipes)] [2] [2].id = strint (temp.Substring(4,2));
    //Gets the id and amount of the output and inserts directly into the outputs array
    get(stream, ref outputs [upper (outputs)].id);
    get(stream, ref outputs [upper (outputs)].amount);
}
//Closes to prevent errors and as good cleanup
close(stream);
...
var window = Window.Open ("graphics:801;641, offscreenonly, title: 2D-Craft");
// Saves the root directory of the program for later uses
            maindirectory = Dir.Current();
...
var updatePart = 0;
// Loads images
var terrain = Pic.FileNew ("images/terrain.gif");
var terrainmodified = Pic.FileNew ("images/modifiedterrain.bmp");
var items = Pic.FileNew ("images/items.gif");
var fullheart = Pic.FileNew ("images/full-heart.gif");
var halfheart = Pic.FileNew ("images/half-heart.gif");
var mobfire = Pic.FileNew ("images/fire.gif");
var inventoryscreen = Pic.FileNew ("images/inventory.gif");
var craftingscreen = Pic.FileNew ("images/crafting.gif");
var background = Pic.FileNew ("images/Sky.gif");
// Draws texture pack
Pic.Draw (terrain, 0, 0, picCopy);
//Array of breaking animation frames
int[] breakinganimations=new int[10];
//2D array of textures, 11 light levels for each texture.
int[,] textures =new int[255,11];//255,10
//Miniature textures of the above
int[] smalltextures =new int[255];
//Array of block information
blockinfo[] blockinfos =new blockinfo[257];
...
open( stream, "datafiles/blocks.txt", "r");
//Temporary storage variables
string tooltyperequired, tool, hardness2, weakness, name, id, nums;
int num;
...
int[,] opendoortextures =new int[2,11];
...
while(true){
    ...
    id = id.Substring(5,length(id));
    //Skips the next 2 lines(x, y) as they are irrevelant in this(used for texturepack creation)
    get(stream, ref temp);
    get(stream, temp);
    //Reads in the name
    get(stream, ref name);
    name = name.Substring(7,length(name));
    //Reads in the hardness
    get(stream, ref hardness2);
    hardness2 = hardness2.Substring(11,length(hardness2));
    //Reads in the weakness
    get(stream, ref weakness);
    weakness = weakness.Substring(11,length(weakness));
    //Reads in the tooltyperequired boolean
    get(stream, ref tooltyperequired);
    tooltyperequired = tooltyperequired.Substring(19,length(tooltyperequired));
    //Reads in the tool boolean
    get(stream, ref tool);
    tool = tool.Substring(7,length(tool));
    ...
    for(int i=0;i<=10;i++){
        ...
        if (name == "DoorBottom")
            opendoortextures [0,i] = Pic.New (i * 16, num * 16 - 16, i * 16 + 2, num * 16 - 1);
        else if (name == "DoorTop") {
...
while(true){
    ...
    get(stream, ref strength);
    strength = strength.Substring(11,length(strength));
    //Reads in the tooltype
    get(stream, ref tooltype);
    tooltype = tooltype.Substring(11,length(tooltype));
    //Reads in the tool boolean
    get(stream, ref tool);
    tool = tool.Substring(7,length(tool));
    //Reads in the durability and placable variables
    get(stream, ref durability);
    get(stream, ref placable);
    placable = placable.Substring(11,length(placable));
    durability = durability.Substring(13,length(durability));
...
int[] blockEntityImages =new int[upper(textures)];
// Loops through the block entities
for (int i=0;i<upper(blockEntityImages);i++){
...
var playerskin = Pic.FileNew ("images/char.gif");
//Draws the character skin
Pic.Draw (playerskin, 0, 0, picMerge);
//Creates base images
var rightleg = Pic.New (0, 0, 3, 11);
var leftleg = Pic.New (8, 0, 11, 11);
var rightarm = Pic.New (40, 0, 43, 11);
var leftarm = Pic.New (48, 0, 51, 11);
var rightbody = Pic.New (16, 0, 19, 11);
var leftbody = Pic.New (28, 0, 31, 11);
var righthead = Pic.New (0, 16, 7, 23);
var lefthead = Pic.New (16, 16, 23, 23);
cls();
...
var zombieskin = Pic.FileNew ("images/zombie.gif");
//Draws the zombie skin
Pic.Draw (zombieskin, 0, 0, picMerge);
//Creates base images
var zombierightleg = Pic.New (0, 0, 3, 11);
var zombieleftleg = Pic.New (8, 0, 11, 11);
var zombierightarm = Pic.New (40, 0, 43, 11);
var zombieleftarm = Pic.New (48, 0, 51, 11);
var zombierightbody = Pic.New (16, 0, 19, 11);
var zombieleftbody = Pic.New (28, 0, 31, 11);
var zombierighthead = Pic.New (0, 16, 7, 23);
var zombielefthead = Pic.New (16, 16, 23, 23);
cls();
// Recreates the moving parts, but with whitespace around them to allow for easier rotation.
...
if (File.Exists ("datafiles/controls")){
    ...
    open(stream, "datafiles/controls", "r");
    get(stream, ref leftkey);
    get(stream, ref temp);
    get(stream, ref rightkey);
    get(stream, ref temp);
    get(stream, ref jumpkey);
    get(stream, ref temp);
    get(stream, ref inventorykey);
    get(stream, ref temp);
    get(stream, ref sprintkey);
    // Closes file to prevent errors
    close (stream);
}
...
if (File.Exists ("datafiles/menumessages")){
    open(stream, "datafiles/menumessages","r");
    while(true){
        if( eof (stream)) break;
        menumessages.Add("");
        get (stream, ref menumessages [upper (menumessages)]);
    }}
...
var blockentities =new blockentity[1000];
...
var craftingtable =new item[3,3];
for (int i=0;i<3;i++){
    for (int j=0;j<3;j++){
...
var breakingblockx = 0;
var breakingblocky = 0;
...
loop
    ...
    const movespeed := 32
    //Sets the adjusted movement speed
    var movespeedadj := movespeed
    ...
    loop
        ...
        if (getblockfloor (positionx, floor (positiony)).blockid = 28 or getblockfloor (positionx + 7, floor (positiony)).blockid = 28) or
                (collision and (getblockfloor (positionx, floor (positiony) - 1).blockid = 28 or getblockfloor (positionx + 7, floor (positiony) - 1).blockid = 28) and chars (downclimbkey)) then
            //Ladder Climbing
            if chars (upclimbkey) then
                if (((getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid not= 0
                        and getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid not= 15
                        and getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid not= 28)
                        and ((getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid = 18
                        and getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).dooropen = 0) = false
                        and (getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid = 19
                        and getblockfloor (positionx, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).dooropen = 0) = false))
                        or ((getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid not= 0
                        and getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid not= 15
                        and getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid not= 28)
                        and ((getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid = 18
                        and getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).dooropen = 0) = false
                        and (getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).blockid = 19
                        and getblockfloor (positionx + 7, floor (positiony + (terminalvelocity / 4) * (timeframe / 1000)) + 29).dooropen = 0) = false))) = false then
                    positiony += (terminalvelocity / 4) * (timeframe / 1000)
                end if
            end if
            if chars (downclimbkey) then
                if (((getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid not= 0
                        and getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid not= 15
                        and getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid not= 28)
                        and ((getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid = 18
                        and getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).dooropen = 0) = false
                        and (getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid = 19
                        and getblockfloor (positionx, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).dooropen = 0) = false))
                        or ((getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid not= 0
                        and getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid not= 15
                        and getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid not= 28)
                        and ((getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid = 18
                        and getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).dooropen = 0) = false
                        and (getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).blockid = 19
                        and getblockfloor (positionx + 7, floor (positiony - (terminalvelocity / 4) * (timeframe / 1000))).dooropen = 0) = false))) = false then
                    positiony -= (terminalvelocity / 4) * (timeframe / 1000)
                end if
            end if
            yvelocity := 0
            ...
            if ((getblockfloor (positionx, floor (positiony + yvelocity * (timeframe / 1000))).blockid not= 0
                ...
                //If the user has hit their head, drop their velocity to 0 so they begin falling again
                    ...
                    and getblockfloor (positionx, floor (positiony + yvelocity * (timeframe / 1000)) + 29).blockid not= 28)
                    ...
                    and getblockfloor (positionx + 7, floor (positiony + yvelocity * (timeframe / 1000)) + 29).blockid not= 28)
        ...
        if chars (rightkey) then
            ...
            if getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).blockid = 0 or
                    getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).blockid = 15 or
                    getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).blockid = 28
                    or ((getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).blockid = 18
                    and getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).dooropen = 0) = true
                    or (getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).blockid = 19
                    and getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony)).dooropen = 0) = true) then
                // If the block at the players torso after moving is air, torch, or open door then continue
                if getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).blockid = 0 or
                        getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).blockid = 15 or
                        getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).blockid = 28
                        or ((getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).blockid = 18
                        and getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).dooropen = 0) = true
                        or (getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).blockid = 19
                        and getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 15).dooropen = 0) = true) then
                    // If the block at the players head after moving is air, torch, or open door then continue
                    if getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).blockid = 0 or
                            getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).blockid = 15 or
                            getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).blockid = 28
                            or ((getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).blockid = 18
                            and getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).dooropen = 0) = true
                            or (getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).blockid = 19
                            and getblockfloor (round (positionxreal + movespeedadj * (timeframe / 1000)) + 7, round (positiony) + 29).dooropen = 0) = true) then
                        // If after moving the player is in an outer chunk then
                        if floor (round (positionxreal + (movespeedadj * (timeframe / 1000))) / 16 / 32) - base = 8 then
                        ...
                        positionxreal += movespeedadj * (timeframe / 1000)
                    end if
                end if
            end if
        end if
        ...
        if chars (leftkey) then
            // If the block at the players feet after moving is air, torch, or open door then continue
            if getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).blockid = 0 or
                    getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).blockid = 15 or
                    getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).blockid = 28
                    or ((getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).blockid = 18
                    and getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).dooropen = 0) = true
                    or (getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).blockid = 19
                    and getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony)).dooropen = 0) = true) then
                // If the block at the players torso after moving is air, torch, or open door then continue
                if getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).blockid = 0 or
                        getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).blockid = 15 or
                        getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).blockid = 28
                        or ((getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).blockid = 18
                        and getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).dooropen = 0) = true
                        or (getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).blockid = 19
                        and getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 15).dooropen = 0) = true) then
                    // If the block at the players head after moving is air, torch, or open door then continue
                    if getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).blockid = 0 or
                            getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).blockid = 15 or
                            getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).blockid = 28
                            or ((getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).blockid = 18
                            and getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).dooropen = 0) = true
                            or (getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).blockid = 19
                            and getblockfloor (round (positionxreal - movespeedadj * (timeframe / 1000)), round (positiony) + 29).dooropen = 0) = true) then
                        // If after moving the player is in an outer chunk then
                        if floor (round (positionxreal - movespeedadj * (timeframe / 1000)) / 16 / 32) - base = 2 then
                            //Save the game to prevent incomplete save files
                            saveGame (mapname)
        ...
        //IF the player is holding the sprint key
        if chars (sprintkey) then
            sprinting := true
            movespeedadj := movespeed * 2
        else
            sprinting := false
            movespeedadj := movespeed
        end if
        //If the player presses e open the inventory
        if chars (inventorykey) and eheld = false then
                ...
                if (bottomblocky + j) div 16 >= 1 and (bottomblocky + j) div 16 <= 128 then
                    ...
                    if getblockfloor (((leftblockx + (i - xoffset))), ((bottomblocky + (j - yoffset)))).blockid not= 0 then
                        ...
                        if getblockfloor (leftblockx + (i - xoffset), bottomblocky + (j - yoffset)).blockid = 15
                                ...
                                or getblockfloor (leftblockx + (i - xoffset), bottomblocky + (j - yoffset)).blockid = 27
                                or getblockfloor (leftblockx + (i - xoffset), bottomblocky + (j - yoffset)).blockid = 28 then
        ...
        for i : 1 .. 9
            ...
            if inventory (i).id > 0 then
                ...
                if inventory (i).id = 11 or inventory (i).id = 13 or inventory (i).id = 14 or inventory (i).id = 128 or inventory (i).id = 15 or inventory (i).id = 27 or inventory (i).id = 28
                        or (inventory (i).id >= 20
        ...
        private static void get(int stream, ref int p)
        {
